...
package transportservice.transport;
...
public final class TransportKeepAlive implements Closeable {
    ...
    private final AsyncBiFunction<transportservice.transport.TcpChannel, BytesReference, Void> pingSender;

    public TransportKeepAlive(ThreadPool threadPool, AsyncBiFunction<transportservice.transport.TcpChannel, BytesReference, Void> pingSender) {
    ...
    public void registerNodeConnection(List<transportservice.transport.TcpChannel> nodeChannels, ConnectionProfile connectionProfile) {
        ...
        for (transportservice.transport.TcpChannel channel : nodeChannels) {
     ...
     */
    void receiveKeepAlive(transportservice.transport.TcpChannel channel) {
    ...
    private void sendPing(transportservice.transport.TcpChannel channel) {
    ...
    private class ScheduledPing extends AbstractLifecycleRunnable {
        ...
        private final Set<transportservice.transport.TcpChannel> channels = ConcurrentCollections.newConcurrentSet();
        ...
        void addChannel(transportservice.transport.TcpChannel channel) {
            channels.add(channel);
        }

        void removeChannel(transportservice.transport.TcpChannel channel) {
            channels.remove(channel);
        }

        @Override
        protected void doRunInLifecycle() {
            for (transportservice.transport.TcpChannel channel : channels) {
        ...
        private boolean needsKeepAlivePing(transportservice.transport.TcpChannel channel) {
            transportservice.transport.TcpChannel.ChannelStats stats = channel.getChannelStats();
